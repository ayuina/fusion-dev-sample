    ...
    public class TodoControllerImpl : ITodoController
    {
        ...
        public Task<ICollection<TodoItem>> ListAllTodoItemAsync()
        {
            ICollection<TodoItem> ret = new List<TodoItem>() {
                new TodoItem() { Id = 1, Title = "Task1", Status = "Active", Task = "経費を清算する", UpdatedOn = DateTimeOffset.Now.AddDays(-1), Duedate = DateTime.Now.AddDays(1) },
                new TodoItem() { Id = 2, Title = "Task2", Status = "Done", Task = "出張を申請する", UpdatedOn = DateTimeOffset.Now.AddDays(-2), Duedate = DateTime.Now.AddDays(2) },
                new TodoItem() { Id = 3, Title = "Task3", Status = "Active", Task = "体温を報告する", UpdatedOn = DateTimeOffset.Now.AddDays(-3), Duedate = DateTime.Now.AddDays(3) }
                };
